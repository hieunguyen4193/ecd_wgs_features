...
params.HGDIR="$params.BWA_REPO/hg38_selected.fa"
HGDIR=file(params.HGDIR)

params.hg_type="38"
params.bin_size="1000"
params.window_size="1000000"
params.ref_name="hglft_genome_rpr_map_Budhraja_STM2023_formated12"
params.MIN_COVERAGE_DEPTH=1
...
process Align_bwa {
    cache "deep"; tag "$sample_id"
    // storeDir "$params.OUTDIR/align_bwa"
    publishDir "$params.OUTDIR/align_bwa", mode: "copy"
    ...
    script:
        """
        bwa mem \
            -t ${params.ALIGN_THREAD} \
            -M \
            -R '@RG\\tID:${sample_id}\\tLB:${sample_id}\\tPL:ILLUMINA\\tPM:MINISEQ\\tSM:${sample_id}' \
            ${params.HGDIR} \
            ${sample_id}_R1_trim50.fastq.gz \
            ${sample_id}_R2_trim50.fastq.gz \
            > ${sample_id}.sam
        """
}
...
process SortBam {
    ...
    output:
        tuple sample_id, file("${sample_id}.sorted.bam") into GW_feature_ch, Mpileup_ch, Depth_ch
        tuple sample_id, file("${sample_id}.sorted.bam"), file("${sample_id}.sorted.bam.bai") into IchorCNA_ch
    script:
        """
        samtools sort --threads $params.SAMTOOLSORT_THREAD -m 4G ${BAM[0]} -o ${sample_id}.sorted.bam
...
process Preprocessing {
    ...
    script:
    """
    bash $SRC/preprocess_bam_file.GW.sh ${Bam} . $HGDIR $sample_id $SRC $RESOURCE/${params.ref_name}.bed
    """
}
...
process Calculate_feature {
    ...
    input:
        ...
        file SRC
    output:
        file("${sample_id}_features") into Summary_feature_ch
    script:
    """
...
process Summary_feature {
    cache "deep"; tag "$sample_id"
    publishDir "${params.OUTDIR}/ready-to-use", mode: "copy"
    // storeDir "${params.OUTDIR}/ready-to-use"

    input:
        file("*_features") from Summary_feature_ch.collect()
        file SRC
        file RESOURCE
    output:
        tuple file("EM.csv"), file("FLEN.csv"), file('NUCLEOSOME.csv') into Predict_feature_ch
    script:
    """
        python3 $SRC/summary_feature.py . . $RESOURCE
    """
}
...
process Run_ichorCNA {
    ...
    script:
    """
    centromere_path=$SRC/inst/extdata/GRCh38.GCA_000001405.2_centromere_acen.txt
    normalPanel_path=$SRC/inst/extdata/HD_ULP_PoN_hg38_1Mb_median_normAutosome_median.rds
...
process Summary_tumor_fraction {
    ...
    input:
        ...
        file SRC
    output:
        file("IchorCNA.csv") into Predict_IchorCNA_ch
    script:
    """
...
process Predict {
    ...
    input:
        file("IchorCNA.csv") from Predict_IchorCNA_ch
        tuple file("EM.csv"), file("FLEN.csv"), file('NUCLEOSOME') from Predict_feature_ch
        file SRC
        file RESOURCE
    output:
        file("output.xlsx")
    script:
    """
        python3 $SRC/predict.py . $RESOURCE/model_files . $RESOURCE/rerun_samples 
    """
}
...
process Mpileup {
    ...
    // storeDir "${params.OUTDIR}/Mpileup"

    input:
        tuple sample_id, file(bam) from Mpileup_ch
    output:
        file("${sample_id}.csv") into Summary_Mpileup_ch
    script:
    """
    a=\$(samtools mpileup ${bam} | awk -v X="${params.MIN_COVERAGE_DEPTH}" '\$4>=X' | wc -l)
    echo "$sample_id,\$a"  > ${sample_id}.csv
    """
}

process Summary_Mpileup {
    cache "deep"; tag "$sample_id"
    publishDir "${params.OUTDIR}/summary_Mpileup", mode: "copy"
    // storeDir "${params.OUTDIR}/Summary_Mpileup"

    input:
        file("*") from Summary_Mpileup_ch.collect()
        file SRC
    output:
        file("Mpileup.csv")
    script:
    """
    python3 $SRC/summary_mpileup.py . .
    """
}
...
process Cal_depth {
    ...
    // storeDir "${params.OUTDIR}/Depth"
    
    input:
        set sample_id, file(bam) from Depth_ch
        file SRC
    output:
        file("${sample_id}_depth_info.csv") into Summary_Depth_ch
    
    script:

    """
    python3 $SRC/cal_depth.py $sample_id ${bam} . 32
    """
}

process Summary_Depth {
    cache "deep"; tag "$sample_id"
    publishDir "${params.OUTDIR}/Summary_Depth", mode: "copy"
    // storeDir "${params.OUTDIR}/Summary_Depth"

    input:
        file("*") from Summary_Depth_ch.collect()
        file SRC
    output:
        file("Depth.csv")
    script:
    """
    python3 $SRC/summary_depth.py . .
    """
}
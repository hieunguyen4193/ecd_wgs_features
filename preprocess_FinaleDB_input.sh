...
cat ${inputfrag} | awk -v OFS='\t' '{$1 = "chr"$1;$6 = $4;$4 = $3 - $2; if ($2 > 5){$5 = $1"_"$2"_"$3"_"$4; print $0; $4 = $2 - $3; $5 = $1"_"$2"_"$3"_"$4; print $0}}' > ${outputdir}/${sampleid}.tmp.frag.tsv
# awk '$1 ~ /^chr([1-9]|1[0-9]|2[0-2]|X|Y|MT)$/' ${outputdir}/${sampleid}.tmp.frag.tsv > ${outputdir}/${sampleid}.frag.tsv
awk '$1 ~ /^chr([1-9]|1[0-9]|2[0-2]|X|Y)$/' ${outputdir}/${sampleid}.tmp.frag.tsv > ${outputdir}/${sampleid}_prep.frag.tsv
rm -rf ${outputdir}/${sampleid}.tmp.frag.tsv